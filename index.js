...
function addElements(mySet, arr) {
  ...
  for (const value of arr) {
    mySet.forEach((elem) => mySet.add(value));
  };
...
function filterAndAdd(mySet, arr) {
  ...
  for (const elem of mySet) {
    if (typeof elem === 'number') {
      mySet.delete(elem);   // Якщо елемент є числом, видаляємо його з множини за допомогою методу delete.
      // console.log(elem);
    }
  }
  ...
  for (const value of arr) {
    // console.log(value);
    mySet.add(value);  // за допомогою методу add. Метод add додає елемент до множини лише якщо він ще не присутній у множині.
  };
...
function setToArray(mySet) {
  ...
  // Використовуємо метод filter для створення нового масиву, що містить лише рядкові елементи.
    myArray = myArray.filter((value) => typeof value === 'string');
...
function removeDuplicatesInPlace(arr) {
  ...
  let myUniqueSet = new Set();

  // Перебір елементів масиву за допомогою циклу for від 0 до довжини масиву
  for (let i = 0; i < arr.length ; i++) {
     // Перевірка, чи елемент вже присутній у множині
     if (!myUniqueSet.has(arr[i])) {
      // myUniqueSet = myUniqueSet.delete(arr[i]); // Якщо елемент вже є унікальним, видаляємо його з масиву
      myUniqueSet.add(arr[i]);
  ...
  myUniqueSet.forEach((value) => {arr.push(value)});
...
// Якщо елемент не належить другій множині, додаємо його до differenceSet
    if (!set2.has(elem)) {
      differenceSet = differenceSet.add(elem);
    };
  };
...
// Додавання спільного елементу до множини intersectionSet
  intersectionSet = intersectionSet.add(elem);
...
function iterateSet(set) {
  ...
  for (const elem of keysIterator) {
  ...
  for (const elem of valuesIterator) {
